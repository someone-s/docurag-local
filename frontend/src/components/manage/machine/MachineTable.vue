  ...
  <div class="size-full relative">
    <div class="absolute top-0 left-0 right-0 bottom-0 p-3 flex flex-col">
      <div class="flex items-center py-4 gap-2 flex-wrap">
        <MachineMake :allow-unset="true" variant="outline" v-on:select="(select) => make = select" />
        <MachineCategory :allow-unset="true" variant="outline" v-on:select="(select) => category = select" />
        <Input class="max-w-3xs" placeholder="Model" @update:model-value="(value) => model = value.toString()" />
        <MachineAddPopover v-on:machine-added="() => manualRefresh++" class="ml-auto"/>
